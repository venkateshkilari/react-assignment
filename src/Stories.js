...
export default function () {
  ...
  const [storyData, setStoryData] = useReducer(StoryReducer, {
    type: "",
    complexity: "",
  });
  ...
  const RowTemplate = ({ rowData, dataIndex, rowIndex }) => {
    ...
    return (
      <TableRow key={rowIndex}>
        ...
        {state && state.user && state.user.role === "Admin" && !rowData[7] && (
          <React.Fragment>
            <TableColumn>
              <Button
                variant="contained"
                ...
                color="primary"
                onClick={async () => {
                  await UpdateStory(rowData[0], "accepted", dataIndex);
                }}
              >
                Accept
              </Button>
              <Button
                ...
                onClick={async () => {
                  await UpdateStory(rowData[0], "rejected", dataIndex);
                }}
              >
                Reject
              </Button>
        ...
        {state && state.user && state.user.role === "Admin" && rowData[7] && (
          <React.Fragment>
            <TableColumn style={columnStyle}>{rowData[7]}</TableColumn>
          </React.Fragment>
        )}
      </TableRow>
    );
            ...
            <div className="row">
              <div className="col-sm-12">
                <FormControl>
                  ...
                  <Select
                    labelId="Type"
                    value={storyData.typeDropdown}
                    style={{ width: "300px" }}
                    error={feildErrors.type}
                    onChange={(event) => {
                      setStoryData({
                        type: "type",
                        payload: event.target.value,
            ...
            <div className="row">
              <div className="col-sm-12">
                <FormControl>
                  ...
                  <Select
                    labelId="Complexity"
                    value={storyData.complexityDropdown}
                    error={feildErrors.complexity}
                    style={{ width: "300px" }}
                    onChange={(event) => {
                      setStoryData({
                        type: "complexity",
            ...
            <div className="row">
              <div className="col-sm-12">
                <Input
                  ...
                  style={{ width: "300px" }}
                  onChange={(event) => {
                    setStoryData({ type: "cost", payload: event.target.value });
                  }}
                />
              </div>
            </div>
